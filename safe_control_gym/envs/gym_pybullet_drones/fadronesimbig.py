...
class FAHexarotor(gym.Env):
    def __init__(self, mass = None, J = None, sim_freq = 240, init_xyz = None, init_rpys = None, init_vel = None, init_omega = None, render_flag = False):
        ...
        self.M = 6.77 if mass is None else mass#0.1 #
        self.J = np.diag([1.05, 1.05, 2.05]) if J is None else J#0.25*np.diag([1.0, 1.0, 2.0])#
        ...
        # Action space
        act_lower_bound = np.array([-self.MAX_THRUST, -self.MAX_THRUST, -self.MAX_THRUST, -self.MAX_XY_TORQUE, -self.MAX_XY_TORQUE, -self.MAX_Z_TORQUE])
        act_upper_bound = np.array([self.MAX_THRUST, self.MAX_THRUST, self.MAX_THRUST, self.MAX_XY_TORQUE, self.MAX_XY_TORQUE, self.MAX_Z_TORQUE])
